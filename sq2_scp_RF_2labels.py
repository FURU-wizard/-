...
import random 
random.seed(123)
data_dir = "G:/sq2_scp_ramdom_forest1224/data/bayes 2 label"
data_dir = "G:/sq2_scp_ramdom_forest1224"
# os.mkdir(data_dir)
os.chdir(data_dir)
...
# sum(meta_data["S_value"]<1.1)# 0.84 21

q = 0.7  # 取 top 25% 为 Responsive
thr = meta_data["S_value"].quantile(q)

meta_data["S_Calcium.Response"] = np.where(
    meta_data["S_value"] >= thr,
    "Responsive",
    "Non-Responsive"
)
...
print(meta_data["S_Calcium.Response"].value_counts())
...
rna_scp.obs["S_Calcium.Response"]
#%% sq2 Svalue 阈值划分

meta_data = rna_sq2.obs.copy()
A_value=pd.to_numeric(meta_data['A_20-2.8_A'], errors='coerce')
...
sum(meta_data["S_value"]>1.05)#1.145 21
sum(meta_data["S_value"]<0.97)# 0.84 21

q = 0.6  # 取 top 25% 为 Responsive
thr = meta_data["S_value"].quantile(q)
...
rna_sq2.obs=meta_data.copy()

rna_sq2.obs["S_Calcium.Response"]
...
y_sq2 = rna_sq2_common.obs["S_Calcium.Response"]
y_scp = rna_scp_common.obs["S_Calcium.Response"]
...
thr = np.percentile(var, 20)   # 去掉最低 20%
gene_cols = var[var > thr].index
...
# gene_cols = var[var > 1].index.tolist()
# sum(var>1)# 0.84 21
...
# ===== HVG（在 ComBat 之后，标准化之前）=====

X_hvg_input = X_combat[gene_cols].copy()

gene_mean = X_hvg_input.mean(axis=0)
gene_var  = X_hvg_input.var(axis=0)
...
df_hvg["mean_bin"] = pd.qcut(df_hvg["mean"], q=20, duplicates="drop")

df_hvg["dispersion_norm"] = (
    df_hvg
    .groupby("mean_bin")["dispersion"]
...
hvg_genes = (
    df_hvg
    .sort_values("dispersion_norm", ascending=False)
    .head(N_HVG)
    .index
    .tolist()
)
...
# 构建最终建模矩阵
X_scaled = X_combat[hvg_genes + ["platform_SQ2"]].copy()

# 标准化（只对基因）
scaler = StandardScaler()
X_scaled[hvg_genes] = scaler.fit_transform(X_scaled[hvg_genes])
...
mi = pd.Series(mi, index=hvg_genes)
hvg_genes_final = mi[mi > 0].index
...
# 1. 准备 X / y
# =========================
X_model = X_scaled[hvg_genes_final].copy()
...
param_space = {
    ...
    'max_depth': Integer(70, 110),
    'min_samples_split': Integer(30, 70),
    'min_samples_leaf': Integer(20, 80),
    'max_features': Real(0.2, 0.4),
...
groups = X_scaled.loc[X_train.index, "platform_SQ2"]
...
# opt.class_weight
print("开始贝叶斯优化搜索...")
opt.fit(X_train, y_train,groups=groups)  # 不传 groups，避免空样本错误
...
# print("最佳参数:", opt.best_params_)
print("CV ROC AUC:", opt.best_score_)
